...
describe('Electronics Component', () => {
    ...
    it('should render item', async () => {
        render(<MockJewelery />)
        const itemElements = await screen.findAllByTestId("jewelery-item-0")
        waitFor(() => expect(itemElements).toBeInTheDocument())
    });

    it('Render all 6 items', async () => {
        render(<MockJewelery />)
        const itemElements = await screen.findAllByTestId(/jewelery-item-/i);
    ...
    it('Render the Item Page upon click', async () => {
        render(<MockJewelery />);
        const itemElement = await screen.findByTestId("jewelery-item-0");
