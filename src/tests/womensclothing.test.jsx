...
describe('Electronics Component', () => {
    ...
    it('should render item', async () => {
        render(<MockWomensClothing />)
        const itemElements = await screen.findAllByTestId("womensclothing-item-0")
        waitFor(() => expect(itemElements).toBeInTheDocument())
    });

    it('Render all items', async () => {
        render(<MockWomensClothing />)
        const itemElements = await screen.findAllByTestId(/womensclothing-item-/i);
        expect(itemElements.length).toBeGreaterThan(0)
    });

    it('Render the Item Page upon click', async () => {
        render(<MockWomensClothing />);
        const itemElement = await screen.findByTestId("womensclothing-item-0");
